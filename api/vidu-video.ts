...
export async function POST(req: NextRequest) {
  try {
    ...
    // Tạo session upload
    const fileName = `image-${Date.now()}-${segmentIdx}.png`;
    const session = await createViduUploadSession(fileName, processedImageBuffer.length, "image/png");
    ...
    let status = { state: response.state };
    ...
    const fileName = `vidu-${Date.now()}-${segmentIdx}.mp4`;
